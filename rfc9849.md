...
be all zeros of length `length_of_padding` (see {{padding}}).
...
ECH extension, as described in {{grease-ech}}. 
...
the server. (See {{dont-stick-out}} for an explanation.)  It is also possible for clients to always
...
`maximum_name_length` field as follows, where M is the `maximum_name_length`
value.
...
so clients can implement a new transport connection in any way that is
...
DNS query, as those may reflect inconsistent a different ECH server
...
- Set the `cipher_suite` field to a supported HpkeSymmetricCipherSuite. The
  selection SHOULD vary, so that all supported configurations are exercised,
  but MAY be held constant for successive connections to the same server in the same
  session.
...
### Processing ClientHello after HelloRetryRequest {#client-facing-server-hrr}
...
client implementations MUST NOT use this mode.
...
`ClientHelloInner.random` prevents this attack: because the attacker
