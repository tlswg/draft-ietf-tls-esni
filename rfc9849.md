...
docname: draft-ietf-tls-esni-25
category: std
number: 9849
ipr: trust200902
submissiontype: IETF
...
author:
 -     name: Eric Rescorla
       ins: E. Rescorla
       org: Knight-Georgetown Institute
       email: ekr@rtfm.com
...
EKR: This is correct as-is. 5077 is being referenced for a specific
construct which doesn't have a correspondent in RFC 8466. It's just an
example, and so is fine as informational.
...
fields, such as the Application-Layer Protocol Negotiation (ALPN) list
...
ECH uses Hybrid Public Key Encryption (HPKE) for public key encryption {{RFC9180}}.
...
into an EncodedClientHelloInner structure, defined below:
...
preferred name syntax (see {{Section 2 of RFC9499}}).  That is, to be
...
* A backend server expects an `ECHClientHello.type` of `inner`, and
...
- KEM: DHKEM(X25519, HKDF-SHA256) (see {{Section 7.1 of RFC9180}})
- KDF: HKDF-SHA256 (see {{Section 7.2 of RFC9180}})
- AEAD: AES-128-GCM (see {{Section 7.3 of RFC9180}})
...
information {{RFCYYY1}}. This means that any attacker which can inject
...
I notice that some of these are regular words (zeros) so you have to determine from context whether it's referring to some protocol element or just to the concept "carries an encrypted payload" versus "the payload field". Do you want to take a cut at changing as many of these as make sense and then I can review, or would you prefer I make the changes?

One question is what to do in definition lists. My sense is that the list heds should be non-fixed-width but maybe you have a convention.
...
EKR: Actually, "split mode" except for the first use and any times it's
in a title.


GREASE vs. Grease (IANA Section): GREASE
