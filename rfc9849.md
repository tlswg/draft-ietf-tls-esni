...
the "client-facing server" and the TLS terminator as the "backend server".
...
"server_name" extension instead (see {{server-behavior}}). Then, clients that offer
ECH authenticate the connection with the public name as follows:
...
IANA has created an entry, ech_required (121) in the existing "TLS Alerts" registry (defined in {{!RFC8446}}), with the "DTLS-OK" column set to
"Y".
