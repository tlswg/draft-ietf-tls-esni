...
two differet`ECHConfigContents` values with the same `HpkeKeyConfig` value.
...
If client offers the "encrypted_client-hello" extension and the client-facing
server accepts, then the backend server MAY include an "encrypted_client_hello"
extension in its HelloRetryRequest. The payload of the extension is described in
{{server-hrr}}.
...
client first checks for the presence of the "encrypted_client_hello" extension.
The presence of this extension is an indication that the client-facing server
accepted ECH and that the backend server sent a HelloRetryRequest. If absent,
then the client presumes rejection and proceeds as if the message was sent by
the client-facing server.

The HelloRetryRequest constructed by the backend server is called the
HelloRetryRequestInner message; and the HelloRetryRequest constructed by the
client-facing server is called the HelloRetryRequestOuter message.
...
As described in {{sever-hrr}, the HelloRetryRequestInner is encrypted by the
backend server and included as the payload of the "encrypted_client_hello"
extension of the HelloRetryRequestOuter. The client decrypts the payload using
the HPKE context it shares with the client-facing server. It begins by exporting
a key and nonce for AEAD decryption:

~~~
    hrr_inner_key = context.Export("hrr inner key", Nk)
    hrr_inner_nonce = context.Export("hrr inner nonce", Nn)
~~~

Here `Nk` and `Nn` denote, respectively, the key and nonce length of the AEAD
algorithm associated with `context`. Let `encrypted_hrr_inner` denote the
extension payload. The client computes

~~~
    HelloRetryRequestInner = Open(hrr_inner_key,
                                  hrr_inner_nonce,
                                  "", // Empty AAD
                                  encrypted_hrr_inner)
~~~

If decryption fails, then the client aborts with a "TODO" alert.

The second ClientHelloOuter contains no extensions except for the
"encrypted_client_hello" extension with the second ClientHelloInner. The client
uses HelloRetryRequestInner to construct ClientHelloInner per the rules of
...
ClientHelloOuter (i.e., the "ech_outer_extensions" extension is used). It then
encrypts the new EncodedClientHelloInner value as a second message with the
...
If the client-facing server sent HelloRetryRequestOuter without the
"encrypted_client_hello" extension, then the client presumes rejection and
constructs the second ClientHelloOuter as described in {{RFC8446}}. In
particular, it MUST copy the "encrypted_client_hello" from the first
ClientHelloOuter into the second.
...
using the procedure in {{server-hrr}}, and forwards the resulting second
ClientHelloInner. The client-facing server forwards all other TLS messages
...
The client-facing server begins by exporting a key and nonce for AEAD
encryption:
...
Here `Nk` and `Nn` denote, respectively, the key and nonce length of the AEAD
algorithm associated with `context`. The extension payload is computed as

~~~
    encrypted_hrr_inner = Seal(hrr_inner_key,
                               hrr_inner_nonce,
                               "", // Empty AAD
                               HelloRetryRequestInner)
~~~

The ciphertext `encrypted_hrr_inner` is used as the payload of the
"encrypted_client_hello" extension. When sending this extension, the
HelloRetryRequestOuter MAY also contain a "cookie" extension, but SHOULD NOT
contain any other extension.

After sending HelloRetryRequestOuter, the client-facing server does not repeat
the steps in {{client-facing-server}} with the second ClientHelloOuter. Instead,
it continues with the ECHConfig selection from the first ClientHelloOuter as
follows:
...
1. encrypted_client_hello(0xfe0a), with "TLS 1.3" column values set to
   "CH, HRR, EE", and "Recommended" column set to "Yes". [[TODO: Should "HRR" be
   "SH"?]]
